...
"""
    ...
    for file in args.files:
        if args.namelist:
            P[file] = Namelists(
                file, exclude=args.exclude, ignore_files=args.ignore_files
            )
        elif args.mom6namelist:
            if len(args.ignore_files) > 0:
                P[file] = Namelists(
                    file, exclude=args.exclude, ignore_files=args.ignore_files
                )
            else:
                P[file] = Namelists(
                    file, exclude=args.exclude, ignore_files=["*.000000.out"]
                )
        elif args.log:
            P[file] = Log(file, exclude=args.exclude, ignore_files=args.ignore_files)
        elif args.assume_mom6:
            P[file] = Parameters(
                file, parameter_files=["*" + file], exclude=args.exclude
            )
        else:
            P[file] = Parameters(file, exclude=args.exclude)
    ...
    if args.format == "object":
        ...
        for i, p in enumerate(P.values()):
            if i == 0:
                ...
                if debug:
                    print("Comparing", p0.label, "<-->", p.label)
                diff = p0.compare(p)
                allkeys += diff.keys()
                p0 = p
        ...
        if args.format == "json":
            ...
            if args.transpose:
                for p in allkeys:
                    print("<th><div>" + p + "</div></th>")
                for e in table:
                    print("<tr>\n<td>" + e + "</td>")
                    for p in allkeys:
                        print("<td>" + table[e][p] + "</td>")
                    print("</tr>")
            else:
                for e in table:
                    print("<th><div>" + splitPath(e) + "</div></th>")
                for p in allkeys:
                    print("<tr>\n<td>" + p + "</td>")
                    for e in table:
                        print("<td>" + table[e][p] + "</td>")
...
def openParameterFile(
    file,
    parameter_files=["*MOM_parameter_doc.all", "*MOM_parameter_doc.short", "MOM_input"],
    ignore_files=[],
    debug=False,
):
        ...
        for root, dirs, _ in os.walk(file):
            ...
            for pat in parameter_files:
                ...
                for fm in ignore_files:
                    ...
                    for ti in to_ignore:
                        possible_matches.remove(ti)
                matches += possible_matches
                # matches += glob.glob(root+'/'+pat)
            matches = sorted(matches)
            if len(matches):
                return openParameterFile(
                    matches[0],
    ...
    if not os.path.isfile(file):
        raise Exception("Not sure of file type for " + file)
    ...
    if ext == ".tar":
        ...
        for mp in parameter_files:
            ...
            if len(member):
                if debug:
                    print(
                        "openParameterFile: Found",
                        member[0],
                        "in",
                        tf.getmember(member[0]),
                    )
                return (
                    tf.extractfile(member[0]),
                    file + "(" + member[0] + ")",
        ...
        matches = False
        for pat in parameter_files:
            matches = matches or fnmatch.fnmatch(file, pat)
        if matches:
            if debug:
                print("openParameterFile: opening", file)
...
class Namelists(object):
    ...
    def __init__(
        self,
        file,
        parameter_files=["*.logfile.000000.out", "*logfile.*", "input.nml", "*.nml"],
        exclude=[],
...
class Log(Namelists):
    def __init__(
        self, file, parameter_files=["*logfile.*"], exclude=[], ignore_files=[]
    ):
